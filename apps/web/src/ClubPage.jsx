        ...
        <div className="game-panel__content">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="game-copy mt-3 max-w-2xl text-base">
                Đây là nơi theo dõi tình trạng đội bóng của user: ngân sách,
                điểm rank, đội khởi đầu và đường tắt sang các module phát triển
                đội hình.
              </p>
            </div>
            {clubId ? (
              <span className="game-chip">Club ID #{clubId}</span>
            ) : null}
          </div>

          {loading && (
            <StateBox
              text="Đang tải dữ liệu đội bóng từ service-core..."
              tone="info"
            />
          )}
          ...
          {!loading && !error && team && (
            <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              <InfoCard
                label="Tên CLB người chơi"
                value={team.clubName || "Chưa đặt tên"}
              />
              <InfoCard
                label="Ngân sách"
                value={Number(team.budget || 0).toLocaleString()}
              />
              <InfoCard label="Điểm rank" value={String(team.rankPoint || 0)} />
              <InfoCard
                label="Tactics Team ID"
                value={team.tacticsTeamId || "N/A"}
              />
            </div>
          )}
        ...
        <div className="game-panel__content">
          ...
          <div className="mt-4 space-y-3 text-sm leading-6 text-slate-300">
            <p className="game-stat-card">
              Khi đăng ký, user được gán đội bóng khởi đầu và nhận 22 thẻ cầu
              thủ mùa thường từ câu lạc bộ đã chọn.
            </p>
            <p className="game-stat-card">
              Dữ liệu trên màn này lấy từ hai API thật: session hiện tại và chi
              tiết câu lạc bộ tương ứng theo team đang gắn với user.
            </p>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
              <div className="game-stat-card">
                <p className="game-stat-card__label">Squad Loop</p>
                <p className="mt-2 text-sm text-slate-300">
                  Club {"->"} Players {"->"} Tactics {"->"} Match {"->"} Reward{" "}
                  {"->"} nâng cấp tiếp.
                </p>
              </div>
              <div className="game-stat-card">
