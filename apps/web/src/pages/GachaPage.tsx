...
export function GachaPage() {
  ...
  async function rollOnce() {
    if (!userId) { setRollError('Không tìm thấy user hiện tại để thực hiện roll.'); return }
    if (!selectedBanner) { setRollError('Vui lòng chọn banner trước.'); return }
    setRollError('')
    try {
      const data = await rollMutation.mutateAsync({ userId, bannerCode: selectedBanner.bannerCode })
      setResult(data)
    ...
    <div className="space-y-6">
      ...
      <section className="game-panel game-panel--accent overflow-hidden p-5 sm:p-6">
        <div className="game-panel__content">
          ...
          <p className="game-copy mt-3 max-w-2xl text-base">Chọn banner đang hoạt động, nổ capsule và nhận cầu thủ cùng thông tin pity của bạn.</p>
        </div>
      </section>
          ...
          {selectedBanner && (
            <section className="game-panel game-panel--soft overflow-hidden p-5">
              <div className="game-panel__content">
                ...
                <h3 className="game-title mt-2 text-xl font-bold text-white">{selectedBanner.bannerName}</h3>
                <p className="mt-1 text-xs text-slate-400">
                  Hết hạn: {formatExpiry(selectedBanner.expiredAt)} · Code: <span className="text-slate-300">{selectedBanner.bannerCode}</span>
                </p>
...
function BannerCard({ banner, selected, onSelect, formatExpiry }: {
  banner: GachaBanner
  selected: boolean
  onSelect: () => void
  ...
  const imgSrc = banner.bannerImageUrl
    ? (banner.bannerImageUrl.startsWith('http') ? banner.bannerImageUrl : `${API_BASE_URL}${banner.bannerImageUrl}`)
    : null
  ...
  return (
    ...
    >
      ...
      <div className="relative h-36 w-full overflow-hidden bg-black/40">
        {imgSrc ? (
          <img src={imgSrc} alt={banner.bannerName} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" />
        ) : (
          <div className="flex h-full w-full items-center justify-center text-slate-500 text-xs">Không có ảnh</div>
        )}
        {selected && (
          <div className="absolute inset-0 flex items-center justify-center bg-amber-400/10">
            <span className="rounded-full bg-amber-400 px-3 py-1 text-xs font-bold text-black">Đã chọn</span>
          </div>
        )}
