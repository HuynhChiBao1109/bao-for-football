...
function buildTargetStats(card: UserPlayerCard | null): Record<StatKey, number> {
  ...
  return STAT_KEYS.reduce((acc, key) => ({ ...acc, [key]: Number(card.totalStats[key] ?? 0) }), {} as Record<StatKey, number>)
}

function buildDelta(card: UserPlayerCard, target: Record<StatKey, number>) {
  return STAT_KEYS.reduce((acc, key) => ({ ...acc, [key]: Number(target[key] ?? 0) - Number(card.totalStats[key] ?? 0) }), {} as Record<StatKey, number>)
}
...
export function PlayersPage() {
  ...
  const spendPoints = useMemo(() => STAT_KEYS.reduce((sum, key) => sum + Number(delta[key] ?? 0), 0), [delta])
  ...
  const hasNegativeBonus = useMemo(() => {
    ...
    return STAT_KEYS.some((key) => Number(selectedCard.bonusStats[key] ?? 0) + Number(delta[key] ?? 0) < 0)
  }, [selectedCard, delta])
  ...
  function clampValue(key: StatKey, next: number): number {
    ...
    const maxVal = Number(selectedCard.totalStats[key] ?? 0) + Number(selectedCard.currentPoints ?? 0) - deltaWithoutKey
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setFeedback(null)
    try {
      await allocateMutation.mutateAsync({ playerId: selectedCard.userPlayerId, delta: delta as Record<StatKey, number> })
      setFeedback({ type: 'ok', text: spendPoints >= 0 ? `Đã áp dụng thay đổi (-${spendPoints} điểm).` : `Đã áp dụng thay đổi (+${Math.abs(spendPoints)} điểm hoàn lại).` })
    } catch (err) {
        ...
        <div className="game-panel__content">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <h2 className="game-title mt-3 text-3xl font-bold text-white">Quản lí cầu thủ, cấp độ và chỉ số</h2>
              <p className="game-copy mt-3 max-w-2xl text-base">Chọn thẻ cầu thủ, kiểm tra tiến trình level, cộng kỹ năng và xem toàn bộ chỉ số nền/tăng thêm/tổng trong cùng một bảng điều khiển.</p>
            </div>
            <button type="button" onClick={() => refetch()} className="game-button-secondary">Tải lại</button>
          </div>
          ...
          {!isLoading && cards.length === 0 && <Banner text="Bạn chưa có cầu thủ nào." tone="muted" />}
              ...
              <form className="mt-4 space-y-2" onSubmit={handleSubmit}>
                <div className="grid gap-2 sm:grid-cols-2">
                  {STAT_FIELDS.map(({ key, label }) => (
                    <StatRow
                      ...
                      canIncrease={clampValue(key as StatKey, allocate[key as StatKey] + 1) > allocate[key as StatKey]}
                      canDecrease={clampValue(key as StatKey, allocate[key as StatKey] - 1) < allocate[key as StatKey]}
                    />
                  ))}
                </div>
                ...
                <div className="mt-4 rounded-[18px] border border-white/8 bg-black/20 p-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className={spendPoints > 0 ? 'text-amber-300 font-semibold' : spendPoints < 0 ? 'text-emerald-300 font-semibold' : 'text-slate-300'}>{spendPoints}</span>
                  </div>
                  <div className="flex justify-between mt-1">
                    <span className="text-slate-400">Còn lại sau</span>
                    <span className={projectedPoints < 0 ? 'text-red-400 font-semibold' : 'text-slate-200 font-semibold'}>{projectedPoints}</span>
                  </div>
                </div>

                {hasNegativeBonus && <Banner text="Không thể giảm stat xuống dưới điểm nền." tone="error" />}

                <button type="submit" disabled={!canSubmit || allocateMutation.isPending} className="game-button-primary mt-2 w-full">
...
function StatRow({ label, base, bonus, value, onChange, canIncrease, canDecrease }: {
  label: string
  base: number
  bonus: number
  value: number
  ...
  return (
    <div className="flex items-center gap-2 rounded-[14px] border border-white/8 bg-black/20 px-3 py-2">
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-slate-500">nền {base} +{bonus}</p>
      </div>
      <button type="button" onClick={() => onChange(value - 1)} disabled={!canDecrease} className="w-6 h-6 rounded-lg bg-white/8 text-slate-300 text-sm disabled:opacity-30 hover:bg-white/15">−</button>
      <input
        type="number"
        value={value}
      ...
      />
      <button type="button" onClick={() => onChange(value + 1)} disabled={!canIncrease} className="w-6 h-6 rounded-lg bg-white/8 text-slate-300 text-sm disabled:opacity-30 hover:bg-white/15">+</button>
    </div>
  )
}
