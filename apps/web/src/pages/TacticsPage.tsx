...
type Role = 'GK' | 'LB' | 'LCB' | 'RCB' | 'RB' | 'LM' | 'LCM' | 'CM' | 'RCM' | 'RM' | 'LW' | 'RW' | 'ST' | 'ST2'
...
const PRIMARY_ROLE_POOL: Role[] = ['GK', 'LB', 'LCB', 'RCB', 'RB', 'LM', 'LCM', 'CM', 'RCM', 'RM', 'LW', 'RW', 'ST']
...
function positionEffect(card: UserPlayerCard, position: string): number {
  ...
  const found = card.positions?.find((item) => String(item.position || '').toUpperCase() === normalized)
  if (!found) return 0.5
...
export function TacticsPage() {
  ...
  const formationSlots = useMemo(() => FORMATION_SLOTS[form.formation] ?? FORMATION_SLOTS['4-3-3'], [form.formation])
  ...
  const usedStarterIds = useMemo(() => new Set(starters.map((s) => s.playerId).filter(Boolean) as number[]), [starters])
  ...
  const starterOverall = useMemo(() => starters.reduce((sum, slot) => sum + slot.score, 0), [starters])
  const starterCount = useMemo(() => starters.filter((s) => Boolean(s.card)).length, [starters])
  const starterAverage = useMemo(() => (starterCount ? starterOverall / starterCount : 0), [starterCount, starterOverall])

  async function submitForm(e: React.FormEvent) {
    e.preventDefault()
    setMessage('')
    try {
      const result = await saveMutation.mutateAsync({ teamId: tacticsTeamId, ...form, lineup: lineupPayload })
      setForm(result)
  ...
  function handleAutoFormat() {
    ...
    for (const slot of formationSlots) {
      ...
      for (let i = 0; i < available.length; i += 1) {
        const score = roleScore(available[i], slot.role) * positionEffect(available[i], roleToPosition(slot.role))
        if (score > bestScore) {
          bestScore = score
          bestIndex = i
        }
    ...
    <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
      <article className="game-panel game-panel--accent overflow-hidden p-5 sm:p-6">
        <div className="game-panel__content">
          ...
          {isCardsLoading && <Banner text="Đang tải danh sách cầu thủ để render đội hình sân bóng..." tone="info" />}
          ...
          <div className="mt-6 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={handleAutoFormat} disabled={cards.length === 0} className="game-button-secondary">
                  Auto Format
                </button>
                <button type="button" onClick={saveLineupOnly} disabled={saveMutation.isPending || !tacticsTeamId} className="game-button-secondary">
                  Lưu Lineup
                </button>
              </div>
            </div>
            ...
            <div className="rounded-[14px] border border-emerald-300/30 bg-emerald-500/10 px-4 py-3 text-sm">
              <div className="flex flex-wrap items-center gap-x-6 gap-y-1">
                <span className="text-slate-200">Đội hình chính: <strong className="text-white">{starterCount}/{formationSlots.length}</strong></span>
                <span className="text-slate-200">Overall Sum: <strong className="text-emerald-300">{starterOverall}</strong></span>
                <span className="text-slate-200">Overall Avg: <strong className="text-white">{starterAverage.toFixed(1)}</strong></span>
              </div>
            </div>
                ...
                <div className="relative h-[680px] sm:h-[760px]">
                  {starters.map((slot) => (
                    <div
                      ...
                      onDragLeave={() => setDragOverSlot((prev) => (prev === slot.slotId ? '' : prev))}
                    ...
                    >
                      <div
                        ...
                        onDragStart={(event) => slot.playerId && onDragStart(event, { playerId: slot.playerId, fromSlotId: slot.slotId })}
                      >
                        <img
                      ...
                      <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.14em] text-emerald-100">{slot.label}</p>
                      <p className="max-w-[90px] truncate text-xs font-semibold text-white">{slot.card ? shortName(slot.card.name) : 'Trống'}</p>
                      <p className="text-[11px] text-emerald-200/80">OVR {slot.score || '--'}</p>
                      <p className="text-[10px] text-amber-200/85">Effect x{slot.effect ? slot.effect.toFixed(2) : '--'}</p>
                    </div>
                  ))}
                </div>
              </div>

              <div className="game-stat-card" onDragOver={(event) => event.preventDefault()} onDrop={onDropBench}>
                <div className="mb-3 flex items-center justify-between">
                  <p className="game-stat-card__label">Dự bị</p>
                  <span className="rounded-full border border-white/15 bg-white/5 px-2.5 py-1 text-xs text-slate-200">{benchPlayers.length} cầu thủ</span>
                </div>
                <div className="max-h-[720px] space-y-2 overflow-y-auto pr-1">
                  {benchPlayers.length === 0 && <p className="text-sm text-slate-400">Không còn cầu thủ dự bị.</p>}
                  {benchPlayers.map(({ card, primary, effect, overall }) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-slate-400">{primary} · Effect x{effect.toFixed(2)} · OVR {overall}</p>
                      </div>
                    </div>
                  ))}
                </div>
